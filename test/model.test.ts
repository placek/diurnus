...
test('normalize: migracja v1 przechodzi przez wszystkie wersje', () => {
  const s = normalize({
    v: 1,
    cats: [{ id: 'x', name: 'X', icon: 'circle', color: 'red', parent: null }],
...
test('normalize: v3 z blokami bez pozycji dostaje pozycje i bieżącą wersję', () => {
  const s = normalize({
    v: 3,
    cats: [{ id: 'x', name: 'X', icon: 'circle', color: 'red', parent: null }],
    day: { start: 6, end: 22, bands: [] },
...
test('normalize: migracja zachowuje pozycje swobodne', () => {
  const s = normalize({
    v: 3,
    cats: [],
    day: { start: 6, end: 22, bands: [] },
...
test('normalize: migracja v2 przechodzi przez wszystkie wersje', () => {
  const s = normalize({
    v: 2,
    cats: [],
    day: { start: 6, end: 22, bands: [] },
...
test('normalize: brak stanu daje bieżącą wersję', () => {
  expect(normalize(null).v).toBe(5);
});

test('normalize: v4 podnosi się do bieżącej wersji', () => {
